...
class Identifier():
	...
	def callback(self, data):
		...
		#RED FOR SCARLET
		hsv_red_lower = np.array([0 - self.sensitivity -5, 100, 100])
		hsv_red_upper = np.array([0 + self.sensitivity -5, 255, 255])
		...
		for pic, contour in enumerate(red_contours):
			...
			if(area > 750):
				...
				#print x, y, w, h
				if (w>100 and h>100) and (w<185 and h<225):
					...
					if np.all(faces): 
						...
						self.character = "Scarlet"
						cv2.imwrite('cluedo_character.png', cv_img)
						f = open("cluedo_character.txt", "w")
						f.write("Scarlet")
						f.close()
		...
		#BLUE FOR PEACOCK
		hsv_blue_lower = np.array([90 - self.sensitivity - 10, 100, 100])
		hsv_blue_upper = np.array([90 + self.sensitivity + 10, 255, 255])
		...
		for pic, contour in enumerate(blue_contours):
       	...
       	#ylw FOR MUSTARD
		hsv_ylw_lower = np.array([20, 50, 50])
		hsv_ylw_upper = np.array([40, 200, 200])
		...
		#PURPLE FOR PLUM
		hsv_prple_lower = np.array([150 - self.sensitivity , 30, 30])
		hsv_prple_upper = np.array([150 + self.sensitivity , 255, 255])
		...
		# 	print ("The character identified is " + character)
		# else:
